...
/// GitHub's own mark, for the buttons that leave for github.com.
///
/// An arrow said "somewhere else" and left the reader to guess where; the logo
/// says where. Filled with the text colour, so it dims and lights with the
/// button around it like any glyph would — see `.ghmark`.
#[component]
pub fn GithubMark() -> Element {
    rsx! {
        svg {
            class: "ghmark",
            view_box: "0 0 16 16",
